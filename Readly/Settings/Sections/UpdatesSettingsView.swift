...
/// automatic behaviours. Sparkle owns the update UI itself, so this pane
/// only states what is true and gets out of the way.
struct UpdatesSettingsView: View {
    ...
    VStack(alignment: .leading, spacing: 18) {
      ...
      SettingsCard(title: "Automatic") {
        ...
        SettingsRow(
          ...
          description: "Fetch the update in advance. Installing still waits for you."
        ) {
  ...
  private var lastCheckedDescription: String {
    guard let date = updater.lastCheckedAt else {
      return "Readly has not checked for updates yet."
    }
